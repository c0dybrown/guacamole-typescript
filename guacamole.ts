...
Guacamole.ArrayBufferReader = function (stream) {
   ...
   * @private
   */
  const guac_reader = this;

  // Receive blobs as array buffers
  stream.onblob = function (data) {

    // Convert to ArrayBuffer
    const binary = (window as any).atob(data);
...
Guacamole.ArrayBufferWriter = function (stream) {
  ...
  function __send_blob(bytes) {
    ...
    // Send as base64
    stream.sendBlob((window as any).btoa(binary));
  ...
  this.sendData = function (data) {
    ...
    // Otherwise, send as multiple instructions
      ...
      for (let offset = 0; offset < bytes.length; offset += guac_writer.blobLength)
...
Guacamole.RawAudioPlayer = function RawAudioPlayer(stream, mimetype) {
   ...
   */
  let format = Guacamole.RawAudioFormat.parse(mimetype);
   ...
   * @type {Number}
   */
  let nextPacketTime = context.currentTime;
   ...
   */
  let reader = new Guacamole.ArrayBufferReader(stream);
   ...
   * @constructor
   */
  const SampleArray = (format.bytesPerSample === 1) ? (window as any).Int8Array : (window as any).Int16Array;
   ...
   * @type {Number}
   */
  let maxSampleValue = (format.bytesPerSample === 1) ? 128 : 32768;
   ...
   */
  const joinAudioPackets = function (packets) {
    ...
    let offset = 0;
    const joined = new SampleArray(totalLength);
   ...
   */
  let splitAudioPacket = function splitAudioPacket(data) {
    ...
    const samples = Math.floor(data.length / format.channels);
    const minSplitSamples = Math.floor(format.rate * MIN_SPLIT_SIZE);
    ...
    for (let offset = start; offset < data.length; offset += format.channels) {
   ...
   *     playback queue.
   */
  let pushAudioPacket = function pushAudioPacket(data) {
   ...
   *     queue.
   */
  let shiftAudioPacket = function shiftAudioPacket() {

    // Flatten data in packet queue
    let data = joinAudioPackets(packetQueue);
    if (!data)
      return null;
   ...
   */
  let toAudioBuffer = function toAudioBuffer(data) {

    // Calculate total number of samples
    let samples = data.length / format.channels;
    ...
    const audioBuffer = context.createBuffer(format.channels, samples, format.rate);
...
Guacamole.RawAudioRecorder = function RawAudioRecorder(stream, mimetype) {
  ...
  /**
   * The (window as any) size to use when applying Lanczos interpolation, commonly
   ...
   * @type {Guacamole.RawAudioFormat}
   */
  const format = Guacamole.RawAudioFormat.parse(mimetype);
   ...
   * @type {Number}
   */
  const maxSampleValue = (format.bytesPerSample === 1) ? 128 : 32768;
  ...
  /**
   * Calculates the value of the Lanczos kernal at point x for a given (window as any)
   ...
   *
   * @param {Number} a
   *     The (window as any) size to use for the Lanczos kernel.
   *
   * @returns {Number}
   *     The value of the Lanczos kernel at the given point for the given
   *     (window as any) size.
   */
  const lanczos = function lanczos(x, a) {
   ...
   */
  const interpolateSample = function getValueAt(audioData, t) {
   ...
   *     AudioBuffer.
   */
  const toSampleArray = function toSampleArray(audioBuffer) {
    ...
    // packet accordingly
    const expectedWrittenSamples = Math.round(readSamples * format.rate / audioBuffer.sampleRate);
    const outSamples = expectedWrittenSamples - writtenSamples;

    // Update number of samples written
    writtenSamples += outSamples;

    // Get array for raw PCM storage
    const data = new SampleArray(outSamples * format.channels);
   ...
   * @private
   */
  const beginAudioCapture = function beginAudioCapture() {
   ...
   * @private
   */
  const stopAudioCapture = function stopAudioCapture() {
...
Guacamole.BlobWriter = function BlobWriter(stream) {
   ...
   * @type {Guacamole.ArrayBufferWriter}
   */
  const arrayBufferWriter = new Guacamole.ArrayBufferWriter(stream);
   ...
   */
  const slice = function slice(blob, start, end) {
  ...
  this.sendBlob = function sendBlob(blob) {
     ...
     * @private
     */
    const readNextChunk = function readNextChunk() {
      ...
      const chunk = slice(blob, offset, offset + arrayBufferWriter.blobLength);
...
Guacamole.Client = function (tunnel) {
  ...
  this.setClipboard = function (data) {
    ...
    // Open stream
    const stream = guac_client.createClipboardStream('text/plain');
  ...
  this.disconnect = function () {
    ...
    if (currentState !== STATE_DISCONNECTED
      ...
      // Stop ping
      if (pingInterval)
        (window as any).clearInterval(pingInterval);
  ...
  this.connect = function (data) {
    ...
    pingInterval = (window as any).setInterval(function () {
...
Guacamole.Display = function () {
  ...
  display.style.transformOrigin =
    ...
    (display.style as any).MozTransformOrigin =
    (display.style as any).OTransformOrigin =
    (display.style as any).msTransformOrigin =
    '0 0';

  // Create default layer
  const default_layer = new Guacamole.Display.VisibleLayer(displayWidth, displayHeight);

  // Create cursor layer
  const cursor = new Guacamole.Display.VisibleLayer(0, 0);
  ...
  function __flush_frames() {
    ...
    while (rendered_frames < frames.length) {
      ...
      const frame = frames[rendered_frames];
      if (!frame.isReady())
        break;
  ...
  function scheduleTask(handler, blocked) {
    const task = new Task(handler, blocked);
    tasks.push(task);
    return task;
  }
  ...
  this.createLayer = function () {
    const layer = new Guacamole.Display.VisibleLayer(displayWidth, displayHeight);
  ...
  this.createBuffer = function () {
    const buffer = new Guacamole.Layer(0, 0);
    buffer.autosize = 1;
    return buffer;
  };
  ...
  this.drawBlob = function (layer, x, y, blob) {
    ...
    const task = scheduleTask(function __display_drawBlob() {
  ...
  this.draw = function (layer, x, y, url) {
    ...
    const task = scheduleTask(function __display_draw() {
  ...
  this.play = function (layer, mimetype, duration, url) {
    ...
    video.addEventListener('play', function () {
      ...
      function render_callback() {
        ...
        if (!video.ended)
          (window as any).setTimeout(render_callback, 20);
      }
  ...
  this.scale = function (scale) {
    ...
    display.style.transform =
      (display.style as any).WebkitTransform =
      (display.style as any).MozTransform =
      (display.style as any).OTransform =
      (display.style as any).msTransform =
  ...
  this.flatten = function () {
    ...
    function get_children(layer) {
      ...
      children.sort(function children_comparator(a, b) {
        ...
        const a_element = a.getElement();
        const b_element = b.getElement();
        const position = b_element.compareDocumentPosition(a_element);
    ...
    function draw_layer(layer, x, y) {
      ...
      if (layer.width > 0 && layer.height > 0) {
        ...
        const initial_alpha = context.globalAlpha;
        ...
        // Draw all children
        const children = get_children(layer);
...
Guacamole.Display.VisibleLayer = function (width, height) {
   ...
   * @private
   */
  const __super_resize = this.resize;
   ...
   * @private
   */
  let translate = 'translate(0px, 0px)'; // (0, 0)

  /**
   * The arbitrary matrix component of this layer's transform.
   * @private
   */
  let matrix = 'matrix(1, 0, 0, 1, 0, 0)'; // Identity
  ...
  this.translate = function (x, y) {
    ...
    // Set layer transform
    div.style.transform =
      (div.style as any).WebkitTransform =
      (div.style as any).MozTransform =
      (div.style as any).OTransform =
      (div.style as any).msTransform =
  ...
  this.shade = function (a) {
    layer.alpha = a;
    div.style.opacity = (a / 255.0).toString();
  };
...
Guacamole.Keyboard = function (element) {
   ...
   *                          http://www.w3.org/TR/DOM-Level-3-Events/#events-KeyboardEvent
   */
  const KeydownEvent = function (keyCode, keyIdentifier, key, location) {
    ...
    this.keysym = keysym_from_key_identifier(key, location, null)
    ...
    const prevent_alt = !guac_keyboard.modifiers.ctrl
      && !(navigator && navigator.platform && navigator.platform.match(/^mac/i));

    // Determine whether default action for Ctrl+combinations must be prevented
    const prevent_ctrl = !guac_keyboard.modifiers.alt;
   ...
   */
  const KeypressEvent = function (charCode) {
   ...
   *                          http://www.w3.org/TR/DOM-Level-3-Events/#events-KeyboardEvent
   */
  const KeyupEvent = function (keyCode, keyIdentifier, key, location) {
    ...
    this.keysym = recentKeysym[keyCode]
      || keysym_from_keycode(keyCode, location)
      || keysym_from_key_identifier(key, location, null); // keyCode is still more reliable for keyup when dead keys are in use
  ...
  const keycodeKeysyms = {
    ...
    91: [0xFFEB], // left (window as any) key (hyper_l)
    92: [0xFF67], // right (window as any) key (menu key?)
    93: null,     // select key
    96: [0xFFB0], // KP 0
    97: [0xFFB1], // KP 1
    98: [0xFFB2], // KP 2
   ...
   */
  const key_identifier_sane = function key_identifier_sane(keyCode, keyIdentifier) {
  ...
  this.press = function (keysym) {
    ...
    if (!guac_keyboard.pressed[keysym]) {
      ...
      if (guac_keyboard.onkeydown) {
        const result = guac_keyboard.onkeydown(keysym);
        last_keydown_result[keysym] = result;

        // Stop any current repeat
        (window as any).clearTimeout(key_repeat_timeout);
        (window as any).clearInterval(key_repeat_interval);

        // Repeat after a delay as long as pressed
        if (!no_repeat[keysym])
          key_repeat_timeout = (window as any).setTimeout(function () {
            key_repeat_interval = (window as any).setInterval(function () {
  ...
  this.reset = function () {
    ...
    for (const keysym in guac_keyboard.pressed)
   ...
   */
  const update_modifier_state = function update_modifier_state(e) {

    // Get state
    const state = Guacamole.Keyboard.ModifierState.fromKeyboardEvent(e);
  ...
  function interpret_events() {
    ...
    let handled_event = interpret_event();
    if (!handled_event)
      return false;
   ...
   */
  const release_simulated_altgr = function release_simulated_altgr(keysym) {
   ...
   *     otherwise.
   */
  const interpret_event = function interpret_event() {
    ...
    if (first instanceof KeydownEvent) {
      ...
      if (accepted_events.length > 0) {
        ...
        if (keysym) {
          ...
          const defaultPrevented = !guac_keyboard.press(keysym);
  ...
  element.addEventListener('keydown', function (e) {
    ...
    let keyCode;
    if ((window as any).event) keyCode = (window as any).event.keyCode;
    ...
    // Log event
    const keydownEvent = new KeydownEvent(keyCode, e.keyIdentifier, e.key, getEventLocation(e));
  ...
  element.addEventListener('keypress', function (e) {
    ...
    let charCode;
    if ((window as any).event) charCode = (window as any).event.keyCode;
  ...
  element.addEventListener('keyup', function (e) {
    ...
    const keyupEvent = new KeyupEvent(keyCode, e.keyIdentifier, e.key, getEventLocation(e));
...
Guacamole.Keyboard.ModifierState = function () {
  ...
  /**
   * Whether hyper ((window as any)s key) is currently pressed.
   * @type {Boolean}
   */
  this.hyper = false;
...
Guacamole.Keyboard.ModifierState.fromKeyboardEvent = function (e) {
  ...
  const state = new Guacamole.Keyboard.ModifierState();
...
Guacamole.Layer = function (width, height) {
   ...
   */
  const resize = function resize(newWidth, newHeight) {
  ...
  this.transfer = function (srcLayer, srcx, srcy, srcw, srch, x, y, transferFunction) {
    ...
    for (let i = 0; i < srcw * srch * 4; i += 4) {
      ...
      const dst_pixel = new Guacamole.Layer.Pixel(
        dst.data[i],
        dst.data[i + 1],
        dst.data[i + 2],
        dst.data[i + 3]
...
Guacamole.Mouse.Touchpad = function (element) {
   ...
   * @private
   */
  const guac_touchpad = this;

  /**
   * The distance a two-finger touch must move per scrollwheel event, in
   * pixels.
   */
  this.scrollThreshold = 20 * ((window as any).devicePixelRatio || 1);
   ...
   */
  this.clickMoveThreshold = 10 * ((window as any).devicePixelRatio || 1);
  ...
  let click_release_timeout = null;
  ...
  element.addEventListener('touchend', function (e) {
    ...
    if (gesture_in_progress && e.touches.length === 0) {
      ...
      const button = touch_buttons[touch_count];
      ...
      if (guac_touchpad.currentState[button]) {
        ...
        if (click_release_timeout) {
          (window as any).clearTimeout(click_release_timeout);
      ...
      if (time - last_touch_time <= guac_touchpad.clickTimingThreshold
        ...
        click_release_timeout = (window as any).setTimeout(function () {
  ...
  element.addEventListener('touchstart', function (e) {
    ...
    if (!gesture_in_progress) {
      ...
      const starting_touch = e.touches[0];
  ...
  element.addEventListener('touchmove', function (e) {
    ...
    const delta_x = touch.clientX - last_touch_x;
    ...
    if (touch_count === 1) {
      ...
      const velocity = pixels_moved / (new Date().getTime() - last_touch_time);
...
Guacamole.Mouse.Touchscreen = function (element) {
   ...
   * @private
   */
  let gesture_start_x = null;

  /**
   * The start Y location of a gesture.
   * @private
   */
  let gesture_start_y = null;
   ...
   * be considered a click.
   */
  this.clickMoveThreshold = 16 * ((window as any).devicePixelRatio || 1);
   ...
   * relative to the browser (window as any), as they will be translated based on
   * the touch event target's location within the browser (window as any).
   *
   * @private
  ...
  function finger_moved(e) {
    ...
    const delta_x = touch.clientX - gesture_start_x;
    const delta_y = touch.clientY - gesture_start_y;
  ...
  function end_gesture() {
    (window as any).clearTimeout(click_release_timeout);
    (window as any).clearTimeout(long_press_timeout);
    gesture_in_progress = false;
  }
  ...
  element.addEventListener('touchstart', function (e) {
    ...
    long_press_timeout = (window as any).setTimeout(function () {
      const touch = e.touches[0];
      move_mouse(touch.clientX, touch.clientY);
      click_button('right');
      end_gesture();
...
Guacamole.Object = function guacamoleObject(client, index) {
   ...
   */
  const dequeueBodyCallback = function dequeueBodyCallback(name) {

    // If no callbacks defined, simply return null
    const callbacks = bodyCallbacks[name];
    if (!callbacks)
      return null;
   ...
   */
  const enqueueBodyCallback = function enqueueBodyCallback(name, callback) {

    // Get callback queue by name, creating first if necessary
    let callbacks = bodyCallbacks[name];
    if (!callbacks) {
      callbacks = [];
  ...
  this.onbody = function defaultBodyHandler(inputStream, mimetype, name) {
    ...
    const callback = dequeueBodyCallback(name);
    if (callback)
...
Guacamole.OnScreenKeyboard = function (layout) {
   ...
   * @private
   */
  const ignorePendingMouseEvents = function ignorePendingMouseEvents() {
   ...
   */
  const ScaledElement = function ScaledElement(element, width, height, scaleFont) {
   ...
   */
  const modifiersPressed = function modifiersPressed(names) {
   ...
   *     can be found.
   */
  const getActiveKey = function getActiveKey(keyName) {

    // Get key array for given name
    const keys = osk.keys[keyName];
    if (!keys)
      return null;
   ...
   */
  const press = function press(keyName, keyElement) {
    ...
    if (!pressed[keyName]) {
      ...
      const key = getActiveKey(keyName);

      // Update modifier state
      if (key.modifier) {

        // Construct classname for modifier
        const modifierClass = 'guac-keyboard-modifier-' + getCSSName(key.modifier);

        // Retrieve originally-pressed keysym, if modifier was already pressed
        const originalKeysym = modifierKeysyms[key.modifier];
   ...
   *     The element associated with the given key.
   */
  const release = function release(keyName, keyElement) {
  ...
  this.resize = function (width) {
    ...
    const unit = Math.floor(width * 10 / osk.layout.width) / 10;

    // Resize all scaled elements
    for (let i = 0; i < scaledElements.length; i++) {
      const scaledElement = scaledElements[i];
   ...
   */
  const asKeyArray = function asKeyArray(name, object) {
   ...
   */
  const getCSSName = function getCSSName(name) {
   ...
   */
  const appendElements = function appendElements(element, object, name) {
    ...
    if (object instanceof Array) {
      ...
      for (i = 0; i < object.length; i++)
        appendElements(div, object[i], null);
    ...
    // If a number, create as a gap
      ...
      // Maintain scale
      scaledElements.push(new ScaledElement(div, object, object, null));
    ...
    // If a string, create as a key
      ...
      if (keys) {
        for (i = 0; i < keys.length; i++) {
          ...
          for (let j = 0; j < key.requires.length; j++) {
            const requirement = key.requires[j];
       ...
       */
      const touchPress = function touchPress(e) {
        e.preventDefault();
        ignoreMouse = osk.touchMouseThreshold;
       ...
       *     The touch event being handled.
       */
      const touchRelease = function touchRelease(e) {
        e.preventDefault();
        ignoreMouse = osk.touchMouseThreshold;
       ...
       *     The touch event being handled.
       */
      const mousePress = function mousePress(e) {
        e.preventDefault();
        if (ignoreMouse === 0)
          press(object, keyElement);
      };
       ...
       *     The touch event being handled.
       */
      const mouseRelease = function mouseRelease(e) {
        e.preventDefault();
        if (ignoreMouse === 0)
          release(object, keyElement);
      };
  ...
  appendElements(keyboard, layout.layout, null);
  ...
  this.receive = function (packet) {
    ...
    while (element_end < buffer.length) {
      ...
      if (element_end >= start_index) {
        ...
        const element = buffer.substring(start_index, element_end);
        const terminator = buffer.substring(element_end, element_end + 1);
        ...
        if (terminator === ';') {
          ...
          // Get opcode
          const opcode = element_buffer.shift();
      ...
      const length_end = buffer.indexOf('.', start_index);
      if (length_end !== -1) {

        // Parse length
        const length = parseInt(buffer.substring(element_end + 1, length_end));
        if (isNaN(length))
...
Guacamole.SessionRecording = function SessionRecording(tunnel) {
   ...
   * @type {Number}
   */
  const KEYFRAME_CHAR_INTERVAL = 16384;
   ...
   * @type {Number}
   */
  const KEYFRAME_TIME_INTERVAL = 5000;
   ...
   * @type {Number}
   */
  let charactersSinceLastKeyframe = 0;
   ...
   */
  const playbackTunnel = new Guacamole.SessionRecording._PlaybackTunnel();
   ...
   */
  const playbackClient = new Guacamole.Client(playbackTunnel);
  ...
  tunnel.oninstruction = function handleInstruction(opcode, args) {
    ...
    const instruction = new Guacamole.SessionRecording._Frame.Instruction(opcode, args.slice());
    ...
    if (opcode === 'sync') {
      ...
      const frame = new Guacamole.SessionRecording._Frame(timestamp, instructions);
      frames.push(frame);
   ...
   */
  const toRelativeTimestamp = function toRelativeTimestamp(timestamp) {
   ...
   *     given value.
   */
  const findFrame = function findFrame(minIndex, maxIndex, timestamp) {
    ...
    const midTimestamp = toRelativeTimestamp(frames[midIndex].timestamp);
   ...
   *     replayed.
   */
  const replayFrame = function replayFrame(index) {
   ...
   *     position.
   */
  const seekToFrame = function seekToFrame(index) {
   ...
   * @private
   */
  const continuePlayback = function continuePlayback() {
    ...
    if (currentFrame + 1 < frames.length) {
      ...
      // frame begins
      const nextRealTimestamp = next.timestamp - startVideoTimestamp + startRealTimestamp;

      // Calculate the relative delay between the current time and
      // the next frame start
      const delay = Math.max(nextRealTimestamp - new Date().getTime(), 0);

      // Advance to next frame after enough time has elapsed
      playbackTimeout = (window as any).setTimeout(function frameDelayElapsed() {
        continuePlayback();
      }, delay);
  ...
  this.seek = function seek(position) {
    ...
    const originallyPlaying = recording.isPlaying();
    recording.pause();
  ...
  this.pause = function pause() {
    ...
    if (recording.isPlaying()) {
      ...
      // Stop playback
      (window as any).clearTimeout(playbackTimeout);
...
Guacamole.SessionRecording._Frame.Instruction = function Instruction(opcode, args) {
  ...
  this.getSize = function getSize() {
    ...
    let size = instruction.opcode.length;
...
Guacamole.StringReader = function (stream) {
   ...
   * @type {Guacamole.ArrayBufferReader}
   */
  const array_reader = new Guacamole.ArrayBufferReader(stream);
  ...
  array_reader.ondata = function (buffer) {
    ...
    // Decode UTF-8
    const text = __decode_utf8(buffer);
...
Guacamole.StringWriter = function (stream) {
   ...
   * @type {Guacamole.ArrayBufferWriter}
   */
  const array_writer = new Guacamole.ArrayBufferWriter(stream);
  ...
  function __expand(bytes) {
    ...
    if (length + bytes >= buffer.length) {
      const new_buffer = new Uint8Array((length + bytes) * 2);
  ...
  function __encode_utf8(text) {
    ...
    // Flush buffer
    if (length > 0) {
      const out_buffer = buffer.subarray(0, length);
      length = 0;
      return out_buffer;
    }
...
Guacamole.HTTPTunnel = function (tunnelURL, crossDomain) {
  ...
  const TUNNEL_CONNECT = tunnelURL + '?connect';
  const TUNNEL_READ = tunnelURL + '?read:';
  const TUNNEL_WRITE = tunnelURL + '?write:';

  const POLLING_ENABLED = 1;
  const POLLING_DISABLED = 0;

  // Default to polling - will be turned off automatically if not needed
  let pollingMode = POLLING_ENABLED;

  let sendingMessages = false;
  let outputMessageBuffer = '';
  ...
  const withCredentials = !!crossDomain;
  ...
  function reset_timeout() {
    ...
    (window as any).clearTimeout(receive_timeout);

    // Set new timeout
    receive_timeout = (window as any).setTimeout(function () {
  ...
  this.sendMessage = function () {
    ...
    let message = getElement(arguments[0]);
  ...
  function handleHTTPTunnelError(xmlhttprequest) {
    ...
    const code = parseInt(xmlhttprequest.getResponseHeader('Guacamole-Status-Code'));
      ...
      if (xmlhttprequest.readyState === 3 ||
        ...
        while (elementEnd < current.length) {
          ...
          if (lengthEnd !== -1) {
            ...
            // Parse length
            const length = parseInt(current.substring(elementEnd + 1, lengthEnd));
...
Guacamole.WebSocketTunnel = function (tunnelURL) {
  ...
  if (tunnelURL.substring(0, 3) !== 'ws:'
    ...
    const protocol = ws_protocol[(window as any).location.protocol];
    ...
    // Otherwise, construct absolute from relative URL
      ...
      const slash = (window as any).location.pathname.lastIndexOf('/');
      const path = (window as any).location.pathname.substring(0, slash + 1);
...
Guacamole.ChainedTunnel = function (tunnelChain) {
  ...
  function attach(tunnel) {
     ...
     */
    const failTunnel = function failTunnel(status) {
      ...
      // Get next tunnel
      const next_tunnel = tunnels.shift();
    ...
    tunnel.onstatechange = function (state) {
      ...
      switch (state) {
        ...
        case Guacamole.Tunnel.State.CLOSED:
          if (!failTunnel(null) && chained_tunnel.onstatechange)
            chained_tunnel.onstatechange(state);
          break;
  ...
  this.connect = function (data) {
    ...
    const next_tunnel = committedTunnel ? committedTunnel : tunnels.shift();
...
Guacamole.StaticHTTPTunnel = function StaticHTTPTunnel(url, crossDomain) {
   ...
   */
  const getGuacamoleStatusCode = function getGuacamoleStatusCode(httpStatus) {
